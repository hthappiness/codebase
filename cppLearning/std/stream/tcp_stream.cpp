...
#define __CLOSE_SOCKET(S) ::closesocket(S)
...
#define __CLOSE_SOCKET(S) ::close(S)
...
struct tstream : public iostream {
    ...
    struct server {
        ...
        // Bind and listen
        server(unsigned short port) : server() {
            bind(port); listen();
        }
        ~server() { close(); }
        ...
        bool bind(unsigned long addr, unsigned short port) {
            ...
            _ret = ::bind(sock(), (struct sockaddr*)&svraddr, sizeof(svraddr));
            return _ret != SOCKET_ERROR;
        }

        bool listen(int backlog = 5) {
            _ret = ::listen(sock(), backlog);
            return _ret != SOCKET_ERROR;
        }
        ...
        operator bool() { return _ret != SOCKET_ERROR; }

    private:
        SOCKET _sock = INVALID_SOCKET;
        int _ret = SOCKET_ERROR;
    };
    ...
    tstream(const char *ip, unsigned short port)
